    ...
    const { id } = useParams(); // Get id from URL params for edit mode
        ...
        useEffect(() => {
            const checkAuthorization = async () => {
                const permission = isEditMode ? 'edit-permission' : 'add-permission';
                ...
                // Fetch permission data if in edit mode
                if (isEditMode) {
        ...
        const validateForm = () => {
            // Slug validation: required|string
            if (!slug.trim()) {
            ...
            if (slug.length > 255) {
                setError('Permission slug must not exceed 255 characters');
                setErrorField('slug');
                slugRef.current?.focus();
                return false;
            }
        ...
        const handleSubmit = async (e) => {
            ...
            try {
                ...
                if (isEditMode) {
                    ...
                    response = await window.axios.put(`/api/v1/permission/${id}`, data, {
                        headers: {
                            Authorization: `Bearer ${localStorage.getItem('token')}`
                        }
                    });
                    ...
                    response = await window.axios.post('/api/v1/permission', data, {
                        headers: {
                            Authorization: `Bearer ${localStorage.getItem('token')}`
                        }
                    });
                ...
                if (response.status === 201 || response.status === 200) {
                    setSuccessMessage(isEditMode ? 'Permission updated successfully!' : 'Permission created successfully!');
                    ...
                    // Redirect to permissions list after 2 seconds
                    setTimeout(() => {
                ...
                if (err.response?.status === 422) {
                    ...
                    setError(err.response?.data?.message || `Failed to ${isEditMode ? 'update' : 'create'} permission. Please try again.`);
                }
            } finally {
                if (!successMessage) {
                    setIsLoading(false);
            ...
            <div className="max-w-2xl mx-auto my-8 p-6 bg-white dark:bg-gray-800 rounded-lg shadow-md">
                <div className="mb-6">
                    ...
                    <p className="text-gray-600 dark:text-gray-400 mt-1">
                        {isEditMode ? 'Update the permission details' : 'Add a new permission to the system'}
                    </p>
                </div>
                ...
                <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-6">
                    <div>
                        <label htmlFor="slug" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                            Permission Slug <span className="text-red-500">*</span>
                        </label>
                        <input
                            ref={slugRef}
                            type="text"
                            ...
                            placeholder="Enter permission slug (e.g., users.create, posts.edit)"
                        />
                        {errorField === 'slug' && error && (
                    ...
                    <div>
                        ...
                        <textarea
                            ...
                            rows="4"
                            className={`w-full p-3 border rounded-lg focus:ring-2 outline-none resize-y ${
                                errorField === 'description'
                                    ? 'border-red-500 focus:border-red-500 focus:ring-red-200'
                                    : 'border-gray-300 focus:border-blue-500 focus:ring-blue-200 dark:border-gray-600 dark:bg-gray-700 dark:text-white'
                            }`}
                            placeholder="Enter a brief description of this permission"
                        />
                    ...
                    <div>
                        ...
                        <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">
                            Select the {isEditMode ? 'current' : 'initial'} status for this permission
                        </p>
                    </div>
                    ...
                    <div className="flex gap-4">
                        ...
                        >
                            ...
                            {isLoading ? (isEditMode ? 'Updating...' : 'Creating...') : (isEditMode ? 'Update Permission' : 'Create Permission')}
                        </button>
                        <button
                            type="button"
